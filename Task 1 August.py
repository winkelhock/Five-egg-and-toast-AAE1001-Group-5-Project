...
scenarios = [
        {"total_passengers": 3300, "max_flights_per_week": 13, "time_cost_level": "medium", "fuel_price": 0.85},
        {"total_passengers": 1500, "max_flights": 28, "is_monthly": True, "time_cost_level": "high", "fuel_price": 0.96},
        {"total_passengers": 2250, "max_flights_per_week": 25, "time_cost_level": "low", "fuel_price": 0.78}
    ]

aircraft_params = {
    "A321neo": {"fixed_cost": 1800, "capacity": 200, "fuel_rate": 54, "time_cost": {"low": 10, "medium": 15, "high": 20}},
    "A330-900neo": {"fixed_cost": 2000, "capacity": 300, "fuel_rate": 84, "time_cost": {"low": 15, "medium": 21, "high": 27}},
    "A350-900": {"fixed_cost": 2500, "capacity": 350, "fuel_rate": 90, "time_cost": {"low": 20, "medium": 27, "high": 34}}
}
class AStarPlanner:
        ...
        while 1:
            ...
            if current.x == goal_node.x and current.y == goal_node.y:
                ...
                for idx, scenario in enumerate(scenarios, 1):
                    print(f"\n========== senario{idx} ==========")
                    print(f"Total Trip Time：{current.cost:.2f} minutes")
        
                    for aircraft, params in aircraft_params.items():
                        flights_needed = math.ceil(scenario["total_passengers"] / params["capacity"])
                        total_flight_time = current.cost * flights_needed
                        total_time_cost = total_flight_time * params["time_cost"][scenario["time_cost_level"]]
                        total_fuel_cost = (params["fuel_rate"] * current.cost) * scenario["fuel_price"] * flights_needed
                        total_cost = total_time_cost + total_fuel_cost + (params["fixed_cost"] * flights_needed)
                        if scenario.get("is_monthly"):
       
                            if flights_needed > scenario["max_flights"]:
                                print(f"Aircraft: {aircraft} not viable!")
                            else:
                                print(f"Aircraft: {aircraft}, Total Cost: ${total_cost:.2f}")
                        else:        
                            if flights_needed > scenario["max_flights_per_week"]:
                                print(f"Aircraft: {aircraft} not viable!")
                            else:
                                print(f"Aircraft: {aircraft}, Total Cost: ${total_cost:.2f}")
                ...
                break
            # Remove the item from the open set list                
            del open_set[c_id]
